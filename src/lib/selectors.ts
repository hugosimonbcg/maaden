...
export function assetsForVertical(vertical: VerticalId): Asset[] {
...
export function filterCosts(f: FilterState): CostBenchmarkRow[] {
  return costBenchmarks.filter((r) => {
    ...
    if (!a) return false
    if (a.verticalId !== f.vertical) return false
...
export function filterPortfolio(vertical: VerticalId): PortfolioItem[] {
  return portfolioItems.filter((p) => p.verticalId === vertical)
}

export function filterActions(vertical: VerticalId): ActionItem[] {
